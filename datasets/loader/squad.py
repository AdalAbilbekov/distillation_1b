...
def tokenize(item, tokenizer, encoder_decoder=False):
    ...
    if not encoder_decoder:
        if 'chat' in tokenizer.name_or_path.lower() or "instruct" in tokenizer.name_or_path.lower() or "llama" in tokenizer.name_or_path.lower() or "batok" in tokenizer.name_or_path.lower():
            ...
            if tokenizer.name_or_path == "tiiuae/falcon-7b-instruct":
                ...
                answer_tokens = tokenizer.encode(f"{item['answers_generated'].strip()}", add_special_tokens=False)
        else:
            context_tokens = tokenizer.encode(f"{tokenizer.bos_token}{prompt}", add_special_tokens=False)
...
def get_split(dataset_config, tokenizer, split):
    ...
    dataset = load_dataset('AdalAbilbekov/llama_chat_issai', data_dir='data')
    # dataset = load_dataset('AdalAbilbekov/test_v1', data_dir='data').rename_column('instruction', 'context').rename_column('input', 'question').rename_column('output', 'answers_generated')

    # AdalAbilbekov/test_v1
