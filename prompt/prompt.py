...
def create_chat_prompt(task: str, few_shot: int, **args):
    ...
    chat, sys_prompt = [], json.load(open(f"/data/nvme3n1p1/adal_workspace/distilation_process/llm-recipes/prompt/context.json"))[task]
    # module = __load_module_from_py_file(f"{os.getenv('HOME')}/llm-distillation/prompt/few_shot/{task}.py")
    module = __load_module_from_py_file(f"/data/nvme3n1p1/adal_workspace/distilation_process/llm-recipes/prompt/few_shot/{task}.py")
