...
import pdb
wandb.login(key="d6f39cbe461c04b7594d30e17320008239f80544")
...
def train(model, train_dataloader, eval_dataloader, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, distil_config, dataset_config, teacher_train_dataloader=None, teacher_eval_dataloader=None, fsdp_config=None, local_rank=None, rank=None):
    ...
    if rank == 0:
        wandb.init(
            project=f"finall_llm_distillation_{dataset_config.file.split('/')[-1][:-3]}",
        ...
        with MemoryTrace() as memtrace:
            ...
            for step, batch in enumerate(train_dataloader if not train_config.distillation else zip(train_dataloader, teacher_train_dataloader)):
                ...
                if train_config.distillation:
                    ...
                    # del student_output['past_key_values']
                    # del teacher_output['past_key_values']
                ...
                if rank == 0:
                    if train_config.distillation:
                        wandb.log({
                            "train/train_loss": loss.detach().float(),
                            "train/cross_loss": cross_loss.detach().float(),
                            "train/distil_loss": dist_loss.detach().float(),
                            "train/teacher_loss": teacher_output.loss.detach().float(),
                    ...
                    if rank == 0:
                        ...
                        if train_config.distillation:
                            wandb.log({
                                ...
                                "eval/eval_epoch_loss": eval_epoch_loss,
                                "eval/eval_cross_loss": eval_cross_loss,
                                "eval/eval_dist_loss": eval_dist_loss
                            })
                        else:
                            wandb.log({
                                "eval_ppl": eval_ppl,
